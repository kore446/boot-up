...
const API_KEY = "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";
...
function buildMats(){
  ...
  return{
    ...
    usbBlack:M({color:0x111111,roughness:0.6,metalness:0.1}),neon:M({color:0x00ffcc,emissive:0x00ffcc,emissiveIntensity:1.2,roughness:0.3}),
...
const PROJECT_SYSTEM = `You are BOOT-UP, an expert hardware engineering agent. When given a project description, respond ONLY with a raw JSON object — no markdown, no backticks, no extra text. Format:
{"title":"Project Title","difficulty":"Beginner","time":"2 hours","cost":"$15","components":[{"name":"Arduino Uno","qty":1,"purpose":"main microcontroller"}],"wiring":[{"from":"Sensor VCC","to":"5V","color":"#ff0000"},{"from":"Sensor GND","to":"GND","color":"#222222"}],"steps":[{"n":1,"title":"Step title","desc":"detailed description","code":"// optional Arduino C++ code or empty string"}],"visual":{"boards":["arduino uno"],"sensors":["hc-sr04","oled"]},"tips":["Pro tip 1"]}
...
function initScene(canvas){
  ...
  const neonL=new THREE.PointLight(0x00ffcc,2.5,20);neonL.position.set(3,4,3);neonL.castShadow=true;scene.add(neonL);
  const rimL=new THREE.PointLight(0xff6b35,1.2,15);rimL.position.set(-3,2,-2);scene.add(rimL);
  const topL=new THREE.DirectionalLight(0xffffff,1.2);topL.position.set(2,10,4);topL.castShadow=true;scene.add(topL);
  const grid=new THREE.GridHelper(20,20,0x001a10,0x001a10);grid.position.set(0,-2.5,0);scene.add(grid);
  ...
  const resize=()=>{const p=canvas.parentElement;if(!p)return;renderer.setSize(p.clientWidth,p.clientHeight,false);camera.aspect=p.clientWidth/p.clientHeight;camera.updateProjectionMatrix();};
  resize();window.addEventListener('resize',resize);
  let raf;
  const animate=()=>{raf=requestAnimationFrame(animate);t+=0.016;if(current&&autoRot)current.rotation.y=t*0.35;neonL.position.set(Math.sin(t*0.5)*4,4,Math.cos(t*0.5)*4);renderer.render(scene,camera);};
  animate();
  return{
    mats,
...
function CodeBlock({code}){
  ...
  return(
    <div style={{position:"relative",marginTop:10}}>
      <pre style={{background:"rgba(0,255,204,0.04)",border:"1px solid rgba(0,255,204,0.15)",borderLeft:"3px solid #00ffcc",padding:"12px 14px",fontFamily:"monospace",fontSize:11,color:"#aaffee",overflowX:"auto",borderRadius:2,margin:0,lineHeight:1.7,whiteSpace:"pre-wrap"}}>{code}</pre>
      <button onClick={()=>{navigator.clipboard?.writeText(code);setCopied(true);setTimeout(()=>setCopied(false),1500);}}
        style={{position:"absolute",top:6,right:6,background:"rgba(0,255,204,0.1)",border:"1px solid rgba(0,255,204,0.3)",color:"#00ffcc",fontFamily:"monospace",fontSize:9,padding:"3px 8px",cursor:"pointer",borderRadius:2,letterSpacing:1}}>
        {copied?"✓ COPIED":"COPY"}
      </button>
    </div>
  );
...
function MsgText({text}){
  ...
  useEffect(()=>{
    ...
    ref.current.innerHTML=text.split('\n').map(line=>
      line.replace(/\*\*(.*?)\*\*/g,'<strong style="color:#00ffcc">$1</strong>')
          .replace(/`(.*?)`/g,'<code style="background:rgba(0,255,204,0.12);padding:1px 5px;font-family:monospace;font-size:11px;color:#00ffcc;border-radius:2px">$1</code>')
    ).join('<br/>');
  },[text]);
  return <div ref={ref} style={{lineHeight:1.65}}/>;
}

const DIFF={Beginner:{c:"#00ff88",bg:"rgba(0,255,136,0.1)"},Intermediate:{c:"#ffaa00",bg:"rgba(255,170,0,0.1)"},Advanced:{c:"#ff4444",bg:"rgba(255,68,68,0.1)"}};
...
export default function BootUp(){
  ...
  const[boardInfo,setBoardInfo]=useState({title:"BOOT-UP SYSTEM",specs:"Type any board or sensor to load its 3D model"});
  const[messages,setMessages]=useState([{role:"agent",text:"👋 Welcome to **BOOT-UP**!\n\n**SIM** → type a board or sensor to see its 3D model\n**PROJECT** → describe a project for a full build guide\n\nBoards: Arduino Uno · Mega · Nano · ESP32 · RPi 5 · Pico · STM32 · Teensy\nSensors: HC-SR04 · DHT22 · PIR · MPU6050 · OLED · LCD · Servo · NeoPixel · Relay · RFID · GPS · and more!"}]);
  ...
  const[projLabel,setProjLabel]=useState("BOOT-UP SYSTEM");
  // 3D - both canvases always in DOM, just hidden
  ...
  // Init PROJECT scene on mount (always running, just hidden)
  useEffect(()=>{
  ...
  // When project data arrives, load the first component into proj scene
  useEffect(()=>{
  ...
  const sendChat=useCallback(async()=>{
    ...
    }catch(e){setMessages(p=>[...p,{role:"agent",text:"⚠️ Error: "+e.message}]);}
    finally{setChatLoading(false);}
  },[chatInput,chatLoading]);

  const generateProject=useCallback(async()=>{
    const q=projInput.trim(); if(!q||projLoading)return;
    setProjLoading(true); setProjError(""); setProject(null); setActiveStep(0);
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",
        headers:{"Content-Type":"application/json","x-api-key":API_KEY,"anthropic-version":"2023-06-01"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2500,system:PROJECT_SYSTEM,messages:[{role:"user",content:`Build project: ${q}`}]})
      });
      const data=await res.json();
  ...
  const dot=<span style={{display:"inline-block",width:5,height:5,borderRadius:"50%",background:"#00ffcc",marginRight:6,animation:"pulse 2s infinite"}}/>;

  // ── SHARED CANVAS PANEL (always rendered, visibility toggled) ──────────────
  const SimCanvas=()=>(
    <div style={{position:"absolute",inset:0}}>
      <div style={{position:"absolute",inset:0,backgroundImage:"linear-gradient(rgba(0,255,204,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(0,255,204,0.03) 1px,transparent 1px)",backgroundSize:"40px 40px",pointerEvents:"none"}}/>
      ...
      {[{top:8,left:8,borderWidth:"2px 0 0 2px"},{top:8,right:8,borderWidth:"2px 2px 0 0"},{bottom:8,left:8,borderWidth:"0 0 2px 2px"},{bottom:8,right:8,borderWidth:"0 2px 2px 0"}].map((s,i)=>(
        <div key={i} style={{position:"absolute",width:18,height:18,borderColor:"#00ffcc",borderStyle:"solid",opacity:0.3,...s,pointerEvents:"none"}}/>
      ))}
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"10px 14px",background:"linear-gradient(transparent,rgba(5,8,16,0.96))",pointerEvents:"none"}}>
        ...
        <div style={{opacity:0.6,lineHeight:1.5,fontSize:11}}>{boardInfo.specs}</div>
      </div>
    </div>
  );
  ...
  const ProjCanvas=()=>(
    <div style={{position:"absolute",inset:0}}>
      <div style={{position:"absolute",inset:0,backgroundImage:"linear-gradient(rgba(0,255,204,0.02) 1px,transparent 1px),linear-gradient(90deg,rgba(0,255,204,0.02) 1px,transparent 1px)",backgroundSize:"40px 40px",pointerEvents:"none"}}/>
      <canvas ref={projCanvasRef} style={{position:"absolute",top:0,left:0,width:"100%",height:"100%",cursor:"grab"}}/>
      {[{top:8,left:8,borderWidth:"2px 0 0 2px"},{top:8,right:8,borderWidth:"2px 2px 0 0"},{bottom:8,left:8,borderWidth:"0 0 2px 2px"},{bottom:8,right:8,borderWidth:"0 2px 2px 0"}].map((s,i)=>(
        <div key={i} style={{position:"absolute",width:18,height:18,borderColor:"#00ffcc",borderStyle:"solid",opacity:0.25,...s,pointerEvents:"none"}}/>
      ))}
      {project&&(
        <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"10px 14px",background:"linear-gradient(transparent,rgba(5,8,16,0.96))",pointerEvents:"none"}}>
          <div style={{fontFamily:"Georgia,serif",fontSize:15,color:"#00ffcc",textShadow:"0 0 15px rgba(0,255,204,0.4)",marginBottom:4}}>{project.title}</div>
          <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:4}}>
            {project.difficulty&&<span style={{fontSize:9,padding:"2px 7px",borderRadius:2,border:`1px solid ${ds.c}`,color:ds.c,background:ds.bg,fontFamily:"monospace"}}>{project.difficulty}</span>}
            {project.time&&<span style={{fontSize:9,color:"rgba(0,255,204,0.55)",fontFamily:"monospace"}}>⏱ {project.time}</span>}
            {project.cost&&<span style={{fontSize:9,color:"rgba(0,255,204,0.55)",fontFamily:"monospace"}}>💰 {project.cost}</span>}
          </div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
            {[...(project.visual?.boards||[]),...(project.visual?.sensors||[])].map((name,i)=>(
              ...
              }} style={{fontSize:9,padding:"2px 7px",background:"rgba(0,10,20,0.8)",border:"1px solid rgba(0,255,204,0.3)",color:"rgba(0,255,204,0.8)",borderRadius:2,fontFamily:"monospace",cursor:"pointer",pointerEvents:"all"}}>
                {name}
              </button>
            ))}
          </div>
      ...
      {!project&&(
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}>
          <div style={{fontFamily:"Georgia,serif",fontSize:14,color:"rgba(0,255,204,0.2)",letterSpacing:2}}>GENERATE A PROJECT TO SEE 3D PREVIEW</div>
        </div>
      )}
    </div>
  );
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#050810",fontFamily:"'Courier New',monospace",color:"#c8e6ff",overflow:"hidden"}}>
      ...
      {/* HEADER */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px",borderBottom:"1px solid rgba(0,255,204,0.2)",background:"rgba(0,0,0,0.4)",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:4}}>
          {[["sim","🔩 SIM"],["project","⚡ PROJECT"]].map(([t,label])=>(
            <button key={t} onClick={()=>setTab(t)}
              style={{padding:"6px 18px",background:tab===t?"rgba(0,255,204,0.12)":"transparent",border:"1px solid",borderColor:tab===t?"rgba(0,255,204,0.4)":"rgba(0,255,204,0.1)",color:tab===t?"#00ffcc":"rgba(0,255,204,0.35)",fontFamily:"monospace",fontSize:10,cursor:"pointer",letterSpacing:2,borderRadius:2}}>
              {label}
            </button>
          ))}
        </div>
        <div style={{display:"flex",gap:12,fontSize:10,color:"rgba(0,255,204,0.45)",alignItems:"center"}}>
          {dot}<span>ONLINE</span><span style={{marginLeft:4}}>{clock}</span>
        </div>
      </div>

      {/* BOTH TABS — always rendered, visibility toggled so canvases never unmount */}

      {/* ══ SIM TAB ══ */}
      <div style={{display:tab==="sim"?"grid":"none",gridTemplateColumns:"1fr 1fr",flex:1,minHeight:0,overflow:"hidden"}}>

        {/* LEFT — 3D */}
        <div style={{display:"flex",flexDirection:"column",borderRight:"1px solid rgba(0,255,204,0.2)",minHeight:0,overflow:"hidden",background:"radial-gradient(ellipse at 30% 20%,rgba(0,100,80,0.08) 0%,transparent 60%)"}}>
          <div style={{fontFamily:"monospace",fontSize:9,color:"rgba(0,255,204,0.4)",padding:"6px 14px",borderBottom:"1px solid rgba(0,255,204,0.15)",letterSpacing:3,flexShrink:0}}>// HARDWARE SIMULATION ENGINE</div>
          <div style={{flex:1,position:"relative",overflow:"hidden",minHeight:0}}>
            <SimCanvas/>
          </div>
          <div style={{flexShrink:0,padding:"10px 14px",borderTop:"1px solid rgba(0,255,204,0.15)",background:"rgba(0,10,20,0.85)",display:"flex",gap:8,alignItems:"center"}}>
            ...
            <input value={simInput} onChange={e=>setSimInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&loadObject()}
              placeholder="Arduino Uno, ESP32, RPi 5, DHT22, OLED, Relay, Servo..."
            ...
            <button onClick={loadObject}
              style={{background:"transparent",border:"1px solid #00ffcc",color:"#00ffcc",fontFamily:"monospace",fontSize:10,padding:"7px 14px",cursor:"pointer",letterSpacing:2,borderRadius:2}}
              onMouseEnter={e=>{e.currentTarget.style.background="#00ffcc";e.currentTarget.style.color="#050810";}}
              onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color="#00ffcc";}}>SIM</button>
          </div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",minHeight:0,overflow:"hidden",background:"radial-gradient(ellipse at 70% 80%,rgba(255,107,53,0.05) 0%,transparent 60%)"}}>
          <div style={{fontFamily:"monospace",fontSize:9,color:"rgba(0,255,204,0.4)",padding:"6px 14px",borderBottom:"1px solid rgba(0,255,204,0.15)",letterSpacing:3,flexShrink:0}}>// AI AGENT — ENGINEERING ASSISTANT</div>
          <div ref={chatRef} style={{flex:1,overflowY:"auto",padding:"14px",display:"flex",flexDirection:"column",gap:10,minHeight:0}}>
            {messages.map((msg,i)=>(
              <div key={i} style={{maxWidth:"88%",padding:"10px 13px",borderRadius:2,fontSize:13,alignSelf:msg.role==="user"?"flex-end":"flex-start",
                ...
                borderRight:msg.role==="user"?"3px solid #ff6b35":undefined,animation:"fadeIn 0.3s ease"}}>
                <div style={{fontFamily:"monospace",fontSize:9,marginBottom:4,color:msg.role==="user"?"#ff6b35":"#00ffcc"}}>
            ...
            {chatLoading&&(
              <div style={{alignSelf:"flex-start",padding:"10px 13px",background:"rgba(0,255,204,0.04)",border:"1px solid rgba(0,255,204,0.15)",borderLeft:"3px solid #00ffcc",borderRadius:2}}>
                <div style={{fontFamily:"monospace",fontSize:9,color:"#00ffcc",marginBottom:6}}>⬡ BOOT-UP · THINKING</div>
                <div style={{display:"flex",gap:5}}>{[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:"#00ffcc",animation:`bounce 1.2s ${i*0.2}s infinite`}}/>)}</div>
              </div>
            )}
          </div>
          <div style={{flexShrink:0,padding:"10px 14px",borderTop:"1px solid rgba(0,255,204,0.15)",background:"rgba(0,10,20,0.85)",display:"flex",gap:8,alignItems:"flex-end"}}>
            <textarea value={chatInput} onChange={e=>setChatInput(e.target.value)}
              onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendChat();}}}
              disabled={chatLoading} placeholder="Ask about wiring, pinouts, code, robotics..."
              rows={1} style={{flex:1,background:"rgba(0,255,204,0.04)",border:"1px solid rgba(0,255,204,0.2)",color:"#c8e6ff",fontFamily:"'Courier New',monospace",fontSize:13,padding:"8px 10px",outline:"none",resize:"none",minHeight:38,maxHeight:100,borderRadius:2,lineHeight:1.4,opacity:chatLoading?0.5:1}}/>
            <button onClick={sendChat} disabled={chatLoading}
              style={{background:"transparent",border:"1px solid #00ffcc",color:"#00ffcc",fontFamily:"monospace",fontSize:10,padding:"8px 16px",cursor:chatLoading?"not-allowed":"pointer",letterSpacing:2,height:38,opacity:chatLoading?0.5:1,borderRadius:2}}
              onMouseEnter={e=>{if(!chatLoading){e.currentTarget.style.background="#00ffcc";e.currentTarget.style.color="#050810";}}}
              onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color="#00ffcc";}}>SEND ›</button>
          </div>
        </div>
      </div>
      ...
      <div style={{display:tab==="project"?"flex":"none",flexDirection:"column",flex:1,minHeight:0,overflow:"hidden"}}>
        ...
        {/* Input bar */}
        <div style={{padding:"12px 20px",borderBottom:"1px solid rgba(0,255,204,0.12)",background:"rgba(0,255,204,0.02)",flexShrink:0,display:"flex",gap:10,alignItems:"center"}}>
          <span style={{fontSize:10,color:"#00ffcc",whiteSpace:"nowrap",letterSpacing:2}}>PROJECT ›</span>
          <input value={projInput} onChange={e=>setProjInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&generateProject()}
            ...
            style={{flex:1,background:"rgba(0,255,204,0.04)",border:"1px solid rgba(0,255,204,0.2)",color:"#c8e6ff",fontFamily:"'Courier New',monospace",fontSize:13,padding:"9px 14px",outline:"none",borderRadius:2}}/>
          <button onClick={generateProject} disabled={projLoading}
            style={{background:"rgba(0,255,204,0.08)",border:"1px solid #00ffcc",color:"#00ffcc",fontFamily:"monospace",fontSize:10,padding:"9px 20px",cursor:projLoading?"not-allowed":"pointer",letterSpacing:2,opacity:projLoading?0.6:1,borderRadius:2,whiteSpace:"nowrap"}}
            onMouseEnter={e=>{if(!projLoading){e.currentTarget.style.background="#00ffcc";e.currentTarget.style.color="#050810";}}}
            onMouseLeave={e=>{e.currentTarget.style.background="rgba(0,255,204,0.08)";e.currentTarget.style.color="#00ffcc";}}>
            {projLoading?"BUILDING...":"BUILD ›"}
          </button>
        </div>

        {projError&&<div style={{padding:"8px 20px",background:"rgba(255,50,50,0.08)",borderBottom:"1px solid rgba(255,50,50,0.2)",color:"#ff8888",fontSize:12,flexShrink:0}}>{projError}</div>}

        {/* Loading spinner */}
        {projLoading&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:18}}>
            <div style={{display:"flex",gap:8}}>{[0,1,2,3,4].map(i=><div key={i} style={{width:8,height:8,borderRadius:"50%",background:"#00ffcc",animation:`bounce 1.2s ${i*0.15}s infinite`}}/>)}</div>
            <div style={{fontFamily:"monospace",fontSize:11,color:"rgba(0,255,204,0.4)",letterSpacing:3}}>GENERATING PROJECT BLUEPRINT...</div>
          </div>
        )}
        ...
        {!projLoading&&!project&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,padding:40}}>
            <div style={{fontSize:50,opacity:0.08}}>⚡</div>
            <div style={{fontFamily:"Georgia,serif",fontSize:20,color:"rgba(0,255,204,0.2)",letterSpacing:3}}>TYPE A PROJECT IDEA</div>
            <div style={{fontSize:12,color:"rgba(200,230,255,0.18)",textAlign:"center",lineHeight:2.0,maxWidth:520}}>
              Try: <span style={{color:"rgba(0,255,204,0.35)"}}>"Smart plant watering system" · "RFID door lock" · "Obstacle avoiding robot" · "Weather station with OLED" · "Bluetooth LED controller"</span>
            </div>
          </div>
        )}

        {/* Project result - always render canvas container so ref stays alive */}
        <div style={{display:!projLoading&&project?"grid":"none",gridTemplateColumns:"1fr 1fr",flex:1,minHeight:0,overflow:"hidden"}}>

          {/* LEFT — 3D + components */}
          <div style={{display:"flex",flexDirection:"column",borderRight:"1px solid rgba(0,255,204,0.15)",minHeight:0,overflow:"hidden",background:"radial-gradient(ellipse at 30% 20%,rgba(0,80,60,0.08) 0%,transparent 60%)"}}>
            <div style={{fontFamily:"monospace",fontSize:9,color:"rgba(0,255,204,0.4)",padding:"6px 14px",borderBottom:"1px solid rgba(0,255,204,0.1)",letterSpacing:3,flexShrink:0}}>// 3D PREVIEW · CLICK PART TO SWITCH · DRAG TO ROTATE</div>
            <div style={{flex:"0 0 55%",position:"relative",minHeight:0}}>
              <ProjCanvas/>
            </div>
            <div style={{flex:1,overflowY:"auto",padding:"10px 14px",borderTop:"1px solid rgba(0,255,204,0.1)",background:"rgba(0,0,0,0.3)"}}>
              <div style={{fontSize:9,color:"rgba(0,255,204,0.35)",letterSpacing:3,marginBottom:7,fontFamily:"monospace"}}>// COMPONENTS</div>
              <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:12}}>
                {(project?.components||[]).map((c,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"baseline",gap:8,fontSize:11}}>
                    <span style={{color:"#ff6b35",fontFamily:"monospace",minWidth:22,flexShrink:0}}>×{c.qty}</span>
                    <span style={{color:"#c8e6ff",flexShrink:0}}>{c.name}</span>
                    <span style={{color:"rgba(200,230,255,0.3)",fontSize:10}}>— {c.purpose}</span>
                  </div>
                ))}
              </div>
              {(project?.wiring||[]).length>0&&(
                <>
                  <div style={{fontSize:9,color:"rgba(0,255,204,0.35)",letterSpacing:3,marginBottom:6,fontFamily:"monospace"}}>// WIRING</div>
                  <div style={{display:"flex",flexDirection:"column",gap:4}}>
                    {(project?.wiring||[]).map((w,i)=>(
                      <div key={i} style={{display:"flex",alignItems:"center",gap:7,fontSize:10,padding:"4px 6px",background:"rgba(0,255,204,0.03)",borderRadius:2}}>
                        <span style={{color:"#ff6b35",fontFamily:"monospace",minWidth:100,flexShrink:0}}>{w.from}</span>
                        <span style={{width:7,height:7,borderRadius:"50%",background:w.color||"#888",border:"1px solid rgba(255,255,255,0.15)",display:"inline-block",flexShrink:0}}/>
                        <span style={{color:"#00ffcc",fontFamily:"monospace"}}>{w.to}</span>
                      </div>
                    ))}
                  </div>
                </>
          ...
          <div style={{display:"flex",flexDirection:"column",minHeight:0,overflow:"hidden"}}>
            <div style={{fontFamily:"monospace",fontSize:9,color:"rgba(0,255,204,0.4)",padding:"6px 14px",borderBottom:"1px solid rgba(0,255,204,0.1)",letterSpacing:3,flexShrink:0}}>
              // BUILD GUIDE · {(project?.steps||[]).length} STEPS
            </div>
            <div style={{display:"flex",overflowX:"auto",borderBottom:"1px solid rgba(0,255,204,0.1)",flexShrink:0,background:"rgba(0,0,0,0.2)"}}>
              {(project?.steps||[]).map((s,i)=>(
                <button key={i} onClick={()=>setActiveStep(i)}
                  style={{padding:"7px 16px",background:"transparent",border:"none",borderBottom:activeStep===i?"2px solid #00ffcc":"2px solid transparent",color:activeStep===i?"#00ffcc":"rgba(200,230,255,0.3)",fontFamily:"monospace",fontSize:10,cursor:"pointer",whiteSpace:"nowrap",letterSpacing:1}}>
                  {i+1}
                </button>
              ))}
            </div>
                ...
                return(
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
                      <div style={{width:28,height:28,borderRadius:"50%",background:"rgba(0,255,204,0.1)",border:"1px solid rgba(0,255,204,0.4)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"monospace",fontSize:11,color:"#00ffcc",flexShrink:0}}>{s.n}</div>
                      <div style={{fontFamily:"Georgia,serif",fontSize:14,color:"#00ffcc"}}>{s.title}</div>
                    </div>
                    <div style={{fontSize:13,lineHeight:1.85,color:"#c8e6ff"}}>{s.desc}</div>
                    <CodeBlock code={s.code}/>
                    {activeStep===(project?.steps||[]).length-1&&(project?.tips||[]).length>0&&(
                      <div style={{marginTop:18}}>
                        <div style={{fontSize:9,color:"rgba(255,107,53,0.45)",letterSpacing:3,marginBottom:8,fontFamily:"monospace"}}>// PRO TIPS</div>
                        <div style={{display:"flex",flexDirection:"column",gap:6}}>
                          {project.tips.map((tip,i)=>(
                            <div key={i} style={{fontSize:12,color:"rgba(200,230,255,0.7)",padding:"7px 10px 7px 12px",borderLeft:"2px solid #ff6b35",background:"rgba(255,107,53,0.04)",lineHeight:1.7}}>{tip}</div>
                          ))}
                        </div>
                      </div>
                    )}
            ...
            <div style={{flexShrink:0,padding:"8px 18px",borderTop:"1px solid rgba(0,255,204,0.1)",background:"rgba(0,0,0,0.3)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <button onClick={()=>setActiveStep(s=>Math.max(0,s-1))} disabled={activeStep===0}
                style={{background:"transparent",border:"1px solid rgba(0,255,204,0.2)",color:activeStep===0?"rgba(0,255,204,0.2)":"#00ffcc",fontFamily:"monospace",fontSize:10,padding:"5px 14px",cursor:activeStep===0?"default":"pointer",letterSpacing:1,borderRadius:2}}>← PREV</button>
              <span style={{fontSize:10,color:"rgba(0,255,204,0.3)",fontFamily:"monospace"}}>STEP {activeStep+1} / {(project?.steps||[]).length}</span>
              <button onClick={()=>setActiveStep(s=>Math.min((project?.steps||[]).length-1,s+1))} disabled={activeStep===(project?.steps||[]).length-1}
                style={{background:"transparent",border:"1px solid rgba(0,255,204,0.2)",color:activeStep===(project?.steps||[]).length-1?"rgba(0,255,204,0.2)":"#00ffcc",fontFamily:"monospace",fontSize:10,padding:"5px 14px",cursor:activeStep===(project?.steps||[]).length-1?"default":"pointer",letterSpacing:1,borderRadius:2}}>NEXT →</button>
            </div>
          </div>
        </div>

        {/* Canvas always in DOM even when project is null so ref stays attached */}
        <div style={{display:!projLoading&&project?"none":"none",position:"absolute",pointerEvents:"none",opacity:0,width:1,height:1}}>
      ...
      <style>{`
        ...
        textarea::placeholder,input::placeholder{color:rgba(200,230,255,0.2)}
